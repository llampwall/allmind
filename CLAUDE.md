...
- **PM2 on Windows**: Use `interpreter: 'node'` with `windowsHide: true` for Node.js services. If terminal windows appear despite this config, PM2 likely has a stale cache - run `pm2 delete <service-name>` then `pm2 start config/ecosystem.config.cjs --only <service-name>` to re-add fresh. Python services use `pythonw.exe` (windowless). For .cmd batch files, use `cmd.exe /c` wrapper with `interpreter: 'none'`.
