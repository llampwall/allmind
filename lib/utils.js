...
export const config = {
  ...
  pm2Cmd: process.env.PM2_CMD || 'pm2.cmd',
...
export async function runCmd(cmd, args = [], options = {}) {
  return new Promise((resolve, reject) => {
    const proc = spawn(cmd, args, {
      cwd: options.cwd,
      env: { ...process.env, ...options.env },
      shell: false,  // Don't use shell to avoid percent-escaping and quoting issues
    ...
    // Set timeout (default 30 seconds, configurable via options)
...
/**
 * Run PM2 command (wraps .CMD file with cmd.exe /c for Windows compatibility)
 */
export async function runPm2(args = [], options = {}) {
  // PM2 is a .CMD batch file, so we need to run it via cmd.exe /c
  const cmdArgs = ['/c', config.pm2Cmd, ...args];
  return runCmd('cmd.exe', cmdArgs, options);
}
...
export async function getGitStatus(repoPath) {
  try {
    const [statusResult, branchResult, remoteResult] = await Promise.all([
      runCmd(config.gitPath, ['status', '--porcelain'], { cwd: repoPath }),
      runCmd(config.gitPath, ['branch', '--show-current'], { cwd: repoPath }),
      runCmd(config.gitPath, ['remote', '-v'], { cwd: repoPath }),
    ]);
    ...
    try {
      const abResult = await runCmd(config.gitPath, ['rev-list', '--left-right', '--count', `origin/${branch}...HEAD`], { cwd: repoPath });
...
/**
 * Check if a process is running by PID (Windows-specific)
 */
export async function isProcessRunning(pid) {
  try {
    const result = await runCmd('tasklist', ['/FI', `PID eq ${pid}`, '/NH'], {});
    // If PID is found, tasklist output contains the PID
...
/**
 * Read and evaluate Chinvex status from repo
 * Returns: { status: 'idle'|'ingesting'|'stale'|'error', ...details }
 */
export async function getChinvexStatus(repoPath) {
  const statusFile = join(repoPath, '.chinvex-status.json');
  ...
  // If status is ingesting, check PID staleness
  if (status.status === 'ingesting' && status.pid) {
    const pidAlive = await isProcessRunning(status.pid);
    if (!pidAlive) {
      return {
        status: 'stale',
        message: `Process ${status.pid} no longer running`,
        ...status,
      };
    }
  }
